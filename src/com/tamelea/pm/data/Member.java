...
//	}
	...
	JSONObject makeJSON() {
		...
		JS.addString(val, "familyName", lastName);
		JS.addString(val, "givenName", firstName);
		JS.addString(val, "middleName", middleName);
		JS.addString(val, "previousFamilyName", maidenName);
		JS.addString(val, "nameSuffix", suffix);
		JS.addString(val, "title", title);
		JS.addString(val, "nickName", nickName);
		JS.addDate(val, "dateLastChange", dateLastChange);
		JS.addEnum(val, "sex", sex);
		JS.addDate(val, "dateOfBirth", dateOfBirth);
		JS.addString(val, "placeOfBirth", placeOfBirth);
		JS.addEnum(val, "status", status);
		JS.addBoolean(val, "resident", resident);
		JS.addBoolean(val, "exDirectory", exDirectory);
		JS.addIndex(val, "household", household);
		JS.addIndex(val, "tempAddress", tempAddress);
		JSONArray tarray = new JSONArray();
		for (Transaction transaction: transactions) tarray.add(transaction.makeJSON());
		val.put("transactions", tarray);
		JS.addEnum(val, "maritalStatus", maritalStatus);
		JS.addString(val, "spouse", spouse);
		JS.addDate(val, "dateOfMarriage", dateOfMarriage);
		JS.addString(val, "divorce", divorce);
		JS.addIndex(val, "father", father);
		JS.addIndex(val, "mother", mother);
		JS.addString(val, "eMail", eMail);
		JS.addString(val, "workEMail", workEMail);
		JS.addPhone(val, "mobilePhone", mobilePhone);
		JS.addPhone(val, "workPhone", workPhone);
		JS.addString(val, "education", education);
		JS.addString(val, "employer", employer);
		JS.addString(val, "baptism", baptism);
		JSONArray sarray = new JSONArray();
		for (Service service : services) sarray.add(service.makeJSON());
		val.put("services",  sarray);
		obj.put("value",  val);
		return obj;
	}
}
